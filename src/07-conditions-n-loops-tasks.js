...
function getSumBetweenNumbers(n1, n2) {
  return new Array(n2 - n1 + 1).fill(n1).map((el, i) => el + i).reduce((a, b) => a + b);
}
...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    if (!str.slice(i + 1).includes(str[i]) && !str.slice(0, i).includes(str[i])) return str[i];
  }
  return null;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return (b > a) ? `${openBracket}${a}, ${b}${closeBracket}` : `${openBracket}${b}, ${a}${closeBracket}`;
}
...
function isCreditCardNumber(ccn) {
  const num = ccn.toString().split('').reverse().slice(1);
  const dig = ccn.toString().slice(-1);
  const sum = num
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (open.includes(str[i])) {
      ...
      return false;
    } else if (closePair[str[i]] === stack[stack.length - 1]) {
      stack.pop();
    } else {
      return false;
    }
...
function getCommonDirectoryPath(paths) {
  ...
  let arr = [];
  const parts = paths.map((el) => el.split('/'));

  for (let i = 0; i < parts.length; i += 1) {
    arr = parts[i].filter((el) => base.includes(el));
  }
